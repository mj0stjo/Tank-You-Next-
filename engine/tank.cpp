...
void Tank::update(float deltaTime) {
	...
	if (KeyboardInput::IsPressed('_') && reloadTime <= 0) {
		...
		std::shared_ptr<Bullet> bullet = std::make_shared<Bullet>(bulletSpeed, bulletDamage, direction3, bulletPos, bulletShaderID, "../models/monke.stl", "localBulletc");
		lastBullet = bullet;

		while (ObjectPool::contains(bullet->getName())) {
			bullet->setName(bullet->getName() + "c");
...
bool Tank::onCollissionEnter(std::shared_ptr<GameObject> collissionObj) {
	...
	// look if name contains "netBullet"
